...
export function buildMazeActionPrompt(playerName: string, batchEfficiencyClass: BatchEfficiencyClass): string {
  ...
  return [
    ...
    "Your objective is to reach destinationCell. Each turn, an exit from currentCell that is not yet alreadyExplored",
    "is your move — take it before weighing distance to destinationCell, unless currentCell's cellType is dead-end.",
...
const mazeStructureTool: AgentToolDefinition = {
  ...
  function: {
    ...
    description: [
      ...
      "be last. currentCell is always included because its distance is 0. historyWindowRadius is a fixed configured",
