...
export function buildMazeActionPrompt(playerName: string, batchEfficiencyClass: BatchEfficiencyClass): string {
  ...
  return [
    ...
    "as your own.",
    "Your primary objective is to reach destinationCell with the highest traversal speed. cellType start-cell",
    "and target-cell label the start and destination cells respectively. Each turn, prioritize an openMoves neighbor",
    "from currentCell whose alreadyExplored is false before weighing distance to destinationCell, unless currentCell's",
    "cellType is dead-end. Only dead-end should trigger backtracking.",
    ...
    "with an openMoves neighbor whose alreadyExplored is false; that visited cell is an actual branch target, not a",
    "guess.",
    ...
    "outcome.",
    `A turn with any valid moves costs a constant ${agentBaseDecayUnits}-unit decay charge regardless of how many`,
    `moves it applied. If replay then reaches an invalid move, that adds a`,
    `${agentPartialInvalidPenaltyDecayUnits}-unit penalty, for a total charge of ${partialInvalidTurnCost}. If the very`,
    "first submitted move is already invalid — no progress at all — the turn instead costs a flat",
    `${agentZeroProgressPenaltyDecayUnits}-unit decay charge.`,
