  ...
  const filteredTraversalHistory = currentCell
    ? nextState.traversalHistory
        ...
        .map(({ playerName, row, col, openMoves }) => ({
          playerName,
          cell: { row, col },
          type: openMoves.length <= 1 ? "dead-end" : openMoves.length === 2 ? "corridor" : "junction",
          openMoves: Object.fromEntries(
            openMoves.map((move) => {
              ...
              return [move, { ...neighbor, visited: visitedCells.has(`${neighbor.row},${neighbor.col}`) }]
            }),
          ),
        }))
    : []
...
describe("agent request service", () => {
    ...
    const expectedJsonInput = {
      ...
      messages: [
        ...
        },
        {
          ...
          content:
            "{\"level\":1,\"currentCell\":{\"row\":0,\"col\":0},\"destinationCell\":{\"row\":8,\"col\":7},\"historyWindowRadius\":4,\"filteredTraversalHistory\":[{\"playerName\":\"Self\",\"cell\":{\"row\":0,\"col\":0},\"type\":\"dead-end\",\"openMoves\":{}}]}",
        },
      ],
  ...
  it("detects Ollama thinking responses that include native tool calls", async () => {
    ...
    expect(toolMessages.map(({ content }) => JSON.parse(content) as unknown)).toEqual([
      ...
      {
        suggestedMovesPerTurn: Math.min(
          getNavigationProfile(mazeDimensions).__maxCorridorLength,
          CONFIG.runtime.modelConfig.manhattanDistance,
        ),
