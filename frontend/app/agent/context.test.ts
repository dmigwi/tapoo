...
const expectedAgentPrompt = [
  ...
  "Use openMoves from nearby filteredTraversalHistory entries to build a local map; entries recorded by other players are just as trustworthy as your own.",
  ...
  "A turn with any valid moves costs a constant 1 decay units regardless of how many moves it applied; invalid moves (any moves after the last valid applied move) add a further penalty of 2 decay units on top — the maximum possible in a turn is 3 decay units. A malformed response (invalid JSON, an unknown tool request, or ignoring a duplicate tool call warning) costs a fixed 2 decay units with no moves applied.",
